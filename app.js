...
// A new visitor starts with a clean planner. Their own tasks stay private in their browser.
const defaultTasks = [];
...
function render(){
  ...
  const next=tasks.filter(t=>!t.done).sort((a,b)=>score(b)-score(a))[0];$('#nextTask').textContent=next?next.name:'You’re all caught up!';$('#nextMeta').textContent=next?`${next.subject} · ${formatDue(next.due)}`:'Add a new task to keep your flow.';$('#focusScore').textContent=next?score(next):'—';
}
function renderChart(){const labels=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], today=(new Date().getDay()+6)%7;const hasTasks=tasks.length>0;$('#chart').innerHTML=labels.map((d,i)=>`<div class="bar-wrap"><div class="bar ${i===today?'today':''}" style="height:${hasTasks?[35,58,42,70,48,25,15][i]:5}%"></div><span>${d}</span></div>`).join('')}
